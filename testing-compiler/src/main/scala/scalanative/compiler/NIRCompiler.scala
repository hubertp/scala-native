...
class NIRCompiler(outputDir: File) extends api.NIRCompiler {
  ...
  override def compileAndReport(code: String): Array[api.CompilerError] = {
    ...
    val (global, reporter) = getCompilerWithReporter(ReportedErrors(_), options = ScalaNative)
    val source = new BatchSourceFile(NoFile, code)
    import global._
    val run = new Run
  ...
  object ReportedErrors {
    def apply(settings: Settings): AbstractReporter with ReportedErrors = new StoreErrorsReporter(settings)


    private class StoreErrorsReporter(override val settings: Settings) extends BaseReporter(settings) with ReportedErrors {
    ...
    private case class CompilerError(pos: Int, msg: String) extends api.CompilerError {
  ...
  /**
    ...
    */
  private def getCompilerWithReporter(f: Settings => AbstractReporter with ReportedErrors, options: CompilerOption*): (Global, ReportedErrors) = {
    // I don't really know how I can reset the compiler after a run, nor what else
    // should also be reset, so for now this method creates new instances of everything,
    // which is not so cool.
    //
